...
VC_VALOR_CADENA_2 VARCHAR(300),	-- RUTA DEL ARCHIVO DE IMAGEN
...
DROP PROCEDURE IF EXISTS USP_LISTAR_DOCUMENTOS;
...
BEGIN
	...
	INNER JOIN DOCPLANO_USUARIO_DETALLE DUD ON DUD.IN_ID_DOC_PLANO = DP.IN_ID_DOC_PLANO
	WHERE DUD.CH_ID_USUARIO = ID_USUARIO;
END

DROP PROCEDURE IF EXISTS USP_LISTAR_DOCUMENTOS_ADM;
...
DROP PROCEDURE IF EXISTS USP_VC_NOMBRES;
CREATE PROCEDURE USP_VC_NOMBRES
(IN ID INTEGER)
BEGIN
	SELECT PR.VC_NOMBRE NOM_PROVEEDOR,PY.VC_NOMBRE NOM_PROYECTO,PL.VC_NOMBRE NOM_PLANO
	FROM PROYECTO PY INNER JOIN PROVEEDOR PR
	ON PY.IN_ID_PROVEEDOR = PR.IN_ID_PROVEEDOR
	INNER JOIN PLANO PL ON PY.IN_ID_PROYECTO = PL.IN_ID_PROYECTO
	WHERE PL.IN_ID_PLANO = ID;
END